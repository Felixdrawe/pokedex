function renderSmallCards(pokemonsAllObjects) {
  ...
  for (let i = 0; i < pokemonsAllObjects.length; i++) {
    ...
    smallCards.innerHTML += /*html*/ `
      <div class="pokemon-card-small" onclick="showLargeCard(${pokemonObject.id})"
  ...
  return /*html*/ `
    <div class="pokemon-card-large" style="background-color: ${bgcolors[pokemonObject.types[0].type.name]}">
      <div class="largeCardNav">
        ...
        <img class="cardNavIcons" src="./img/chevright.svg" onclick="nextLargeCard(${pokemonObject.id})">
      </div>
        ...
        <div class="topcontainer-card-large-infos">
          <div class="number-large">
            ...
            <div class="imagecontainer-animation-card-large">
              <img src="https://raw.githubusercontent.com/geekygreek7/animated-pokemon-gifs/master/${pokemonObject.id}.gif">
            </div>
          </div>
          <h3 class="name">${capitalize(pokemonObject.name)}</h3>
          <small class="type">Type:<span> ${pokemonTypes(pokemonObject)}</span></small>
          <small class="type">Abilities:<span> ${pokemonAbilities(pokemonObject)}</span></small>
        </div>
      </div>
